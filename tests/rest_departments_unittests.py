...
class FlaskTestCases(unittest.TestCase):
    ...
    def test_departments_api_get(self):
        ...
        response = tester.post('/api/departments/get', data=dict(login=ADMIN_LOGIN, password=ADMIN_PASSWORD),
    ...
    def test_departments_api_add(self):
        ...
        response = tester.post('/api/departments/add', data=dict(login=ADMIN_LOGIN, password=ADMIN_PASSWORD,
    ...
    def test_departments_add_err(self):
        ...
        for variation in variations:
            response = tester.post('/api/departments/add', data=dict(login=variation[0], password=variation[1],
                                                                     department=department),
                                   follow_redirects=True)
    ...
    def test_departments_api_edit(self):
        ...
        response = tester.post('/api/departments/edit', data=dict(login=ADMIN_LOGIN, password=ADMIN_PASSWORD,
                                                                  department=new_department, id=id),
                               follow_redirects=True)
        response = tester.post('/api/departments/get', data=dict(login=ADMIN_LOGIN, password=ADMIN_PASSWORD),
                               follow_redirects=True)
        self.assertTrue(new_department.encode() in response.data)
        response = tester.post('/api/departments/del', data=dict(login=ADMIN_LOGIN, password=ADMIN_PASSWORD, id=id),
                               follow_redirects=True)
    ...
    def test_departments_api_edit_err(self):
        ...
        response = tester.post('/api/departments/add', data=dict(login=ADMIN_LOGIN, password=ADMIN_PASSWORD,
                                                                 department=department),
                               follow_redirects=True)
        old_response = tester.post('/api/departments/get', data=dict(login=ADMIN_LOGIN, password=ADMIN_PASSWORD),
                                   follow_redirects=True)
        login_vars = (None, '', 'incorrect', User.query.all()[0].login)
        password_vars = (None, '', 'incorrect', User.query.all()[0].password)
        department_vars = (None, '', Departments.query.all()[-1].department)
        ...
        for variation in variations[:-1]:
            response = tester.post('/api/departments/edit', data=dict(login=variation[0], password=variation[1],
                                                                      department=variation[2], id=variation[3]),
                                   follow_redirects=True)
            new_response = tester.post('/api/departments/get', data=dict(login=ADMIN_LOGIN, password=ADMIN_PASSWORD),
                                       follow_redirects=True)
